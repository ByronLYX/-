require('./index.less');
var tempProductFree = require('./index.html');
angular.module('common.components').directive('checkProductFree', function($rootScope, ibssUtils, $timeout, toaster) {
    return {
        priority: 70,
        restrict: 'EA',
        template: tempProductFree,
        scope: {
            detail: "=",
            status: '=',
            recount: '='
                ...
                $scope.detail.products.forEach(function(item, idx) {
                    if (item.cur) {
                        if (item.productId < 100) {
                            ...
                            obj[idx] = ~~item.count;
                            item.startTime = $scope.detail.products.startTime;
                            item.endTime = $scope.detail.products.endTime;
